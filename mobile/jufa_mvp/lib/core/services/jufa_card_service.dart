...
class JufaCardService {
  ...
  Future<List<dynamic>> getCards() async {
    try {
      ...
      if (response.data['success'] == true) {
        ...
        // Sinon chercher cards dans data
        return data['cards'] ?? [];
      } else {
  ...
  Future<Map<String, dynamic>> orderCard({
    ...
    try {
      ...
      if (response.data['success'] == true) {
        // L'API retourne directement {success, message, card_id}
        return {
  ...
  /// Récupérer les prix des cartes
  Future<Map<String, double>> getCardPrices() async {
    try {
      print('📡 Récupération prix cartes: ${AppConfig.apiBaseUrl}/cards/prices');

      final response = await _dio.get(
        '${AppConfig.apiBaseUrl}/cards/prices',
        options: Options(
          headers: {
            'Accept': 'application/json',
          },
      ...
      if (response.data['success'] == true) {
        final data = response.data['data'];
        return {
          'virtual_card_price': (data['virtual_card_price'] as num).toDouble(),
          'physical_card_price': (data['physical_card_price'] as num).toDouble(),
        };
      } else {
        throw Exception(response.data['message'] ?? 'Erreur lors de la récupération des prix');
      }
    } on DioException catch (e) {
      print('❌ Erreur: ${e.message}');
  ...
  Future<double> getReloadFeePercentage() async {
    try {
      print('📡 Récupération frais recharge: ${AppConfig.apiBaseUrl}/fees/reload');

      final response = await _dio.get(
        '${AppConfig.apiBaseUrl}/fees/reload',
        options: Options(
          headers: {
            'Accept': 'application/json',
          },
      ...
      if (response.data['success'] == true) {
        final data = response.data['data'];
        return (data['reload_fee_percentage'] as num).toDouble();
      } else {
        throw Exception(response.data['message'] ?? 'Erreur lors de la récupération des frais');
      }
    } on DioException catch (e) {
      print('❌ Erreur: ${e.message}');
