...
public class B2BOrderService {
    ...
    public OrderResponse createOrder(User user, CreateOrderRequest request) {
        ...
        WholesalerRetailer relation = relationRepository.findByWholesalerAndRetailerAndStatus(
                ...
                .orElseGet(() -> {
                    var existingRelation = relationRepository.findByWholesalerAndRetailer(wholesaler, retailer);
                    if (existingRelation.isPresent()) {
                        var rel = existingRelation.get();
                        rel.setStatus(WholesalerRetailer.RelationStatus.ACTIVE);
                        rel.setApprovedAt(java.time.LocalDateTime.now());
                        log.info("Auto-activated relation between {} and {}", 
                                retailer.getBusinessName(), wholesaler.getBusinessName());
                        return relationRepository.save(rel);
                    }
                    WholesalerRetailer newRelation = WholesalerRetailer.builder()
                            .wholesaler(wholesaler)
                            .retailer(retailer)
                            .status(WholesalerRetailer.RelationStatus.ACTIVE)
                            .creditLimit(java.math.BigDecimal.ZERO)
                            .creditUsed(java.math.BigDecimal.ZERO)
                            .paymentTermsDays(0)
                            .discountRate(java.math.BigDecimal.ZERO)
                            .approvedAt(java.time.LocalDateTime.now())
                            .build();
                    log.info("Auto-created relation between {} and {}", 
                            retailer.getBusinessName(), wholesaler.getBusinessName());
